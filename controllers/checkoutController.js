...
const processPayment = async (req, res) => {
  try {
    ...
    // Validate expiry with enhanced validation
    if (!validateExpiry(expiry)) {
      return res.status(400).send("Invalid or expired card. Please check the expiry date (MM/YY format, month must be 01-12, and card must not be expired)");
    }
